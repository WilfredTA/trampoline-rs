...
pub fn create(name: String, path_to_template: &str) -> Result<()> {
    ...
    for file in TEMPLATES.get_template_names() {
        if file.starts_with("dapp/dapp_template") {
            ...
            let shortened_file_name = file.strip_prefix("dapp/dapp_template/")
                .unwrap();
...
fn is_file<P: AsRef<Path>>(check: P) -> bool {
    ...
    check.ends_with(".ts") || check.ends_with(".js") ||
        check.ends_with(".css") || check.ends_with(".tsx") ||
        check.ends_with(".jsx") || check.ends_with(".svg") ||
        check.ends_with(".md")  || check.ends_with(".html") ||
        check.ends_with(".txt") || check.ends_with(".json") ||
...
fn write_dapp(path_to_template: &PathBuf, name: String) -> Result<()> {
    ...
    let mut cra_res = Command::new("npx")
        .args(["create-react-app", format!("{}", name).as_str(), "--template", format!("file:{}", path_to_template.to_str().unwrap()).as_str()])
