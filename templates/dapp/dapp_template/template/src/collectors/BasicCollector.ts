...
export default class BasicCollector extends Collector {
    ...
    // Collect cells that meet a required
    // capacity with a 
    // lockscript that matches a
    // particular Address.
    async collectCapacity(address: Address, neededAmount: Amount) {
        this.cells = [];
        const indexerQuery = {
            // It seems the ID can be a completely random number
            // Examples in the CKB site show that calls are made with "id: 2"
    ...
    async collectSUDT(sudt: SUDT, address: Address, neededAmount: Amount) {
        ...
        const result = await (
            await fetch(this.indexerUrl, requestOptions)
        ).json();
    ...
    async getCells(address: Address) {
        ...
        // let res = await (await fetch(this.indexerUrl, )).json();
    ...
    async getBalance(address: Address) {
        ...
        const balance = cells.map(
            (c) => c.capacity
        ).reduce(
            (sum, cap) => (sum = sum.add(cap))
        );
